...
static bool new_file    = false; // Inject a file separator?
static bool end_of_file = false; // Inject an end of medium?
...
// On TTYs ctrl+d results in an ASCII end of transmission control character.
...
// Read the ring buffer indices from the microcontroller.
// All four indices ({R, W} x {TX, RX}) are stored in a naturally aligned 32-bit word.
static uint32_t
read_indices_or_die(void)
{
    if ( stlink_read_mem32(handle, addr, 4) ) {
        die("Failed to read the ring buffer indices.");
    }
...
// Write a buffer to the microcontroller 8 bits at a time. Abort on I/O error.
...
// Write a buffer to the microcontroller 32 bits at a time. Abort on I/O error.
...
parse(uint8_t *reply, size_t len)
{
    for ( size_t i = 0; i < len; i++ ) {
        switch ( reply[i] ) {
            ...
            // Begin each new file with file separator marker
            case ASCII_FS:
                line_num = 0;
                break;
        }
...
// Consume everything enqueued by the microcontroller into the ring buffers.
// The ring buffers are single producer single writer queues.
static bool
consume(uint32_t indices)
{
    uint8_t rx_w = (uint8_t)(indices >> 16);
    uint8_t rx_r = (uint8_t)(indices >> 24);
...
// The ring buffers are single producer single writer queues.
static bool
produce(uint32_t indices)
{
    uint8_t tx_w = (uint8_t)(indices >> 0);
    uint8_t tx_r = (uint8_t)(indices >> 8);
    ...
    // On TTYs EOF is signaled with an ASCII end of transmission control character.
...
//
// TTYs have to be put into raw mode because in canonical
...
// Put stdin into non-blocking mode or die trying.
//
...
// Decode the ring buffer indices to stderr.
//
// Calculates how much is free/used in each direction.
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wunused-function"
static void __attribute__((unused))
debug_indices(uint32_t indices)
{
#pragma clang diagnostic pop
    uint8_t tx_w = (uint8_t)(indices >>  0);
    uint8_t tx_r = (uint8_t)(indices >>  8);
...
{
    uint32_t aircr_value = AIRCR_VECTKEY | AIRCR_SYSRESETREQ;

    // Copy the value to the buffer in the stlink handle
...
{
    (void)sig;
    software_reset_or_die();  // Perform software reset instead of hardware reset
}

// Schedule the upload of upload.fs when Ctrl-\ (SIGQUIT) is received.
static void
...
install_signal_handlers(void)
{
    ...
    action->sa_handler = handler_quit; // Handle Ctrl-
...
main(int argc, char *argv[])
{
    ...
    while ( !quit ) {
        uint32_t indices = read_indices_or_die();
        bool rx_active = consume(indices);
        bool tx_active = produce(indices);
